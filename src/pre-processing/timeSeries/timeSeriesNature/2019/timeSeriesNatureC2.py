...
dataset_rows = dataset_columns.loc[dataset_columns['NaturaIncidente'].isin(['Scontro frontale fra veicoli in marcia', 'Scontro laterale fra veicoli in marcia'])]
...
result_data.append({
    'DataOraIncidente':  pd.to_datetime('2019-01-01'),
    'NumeroIncidenti': num_incidents_on_jan_1
})

# Date range da 1 gennaio 2019 a 31 dicembre 2019, con intervallo di 10 giorni
date_range = pd.date_range(start='2019-01-01', end='2019-12-31', freq='10D')
...
result_data[-1]['End Date'] = pd.to_datetime('2019-12-31')

# Creare il DataFrame risultante
result_df = pd.DataFrame(result_data)

# Save the processed DataFrame
result_df.to_csv('dataset/processed/timeSeries/2019/timeSeriesNatureC2.csv', header=True, index=False)
