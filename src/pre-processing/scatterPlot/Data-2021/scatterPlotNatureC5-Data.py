...
dataset_2021PartOne = pandas.concat([pandas.read_csv(file, sep=';', encoding='latin-1') for file in csv_2021PartOne], ignore_index=True)


# Concatenate the filtered CSV files for the complete dataset
for file in csv_2021:
    dataset_2021 = pandas.concat([dataset_2021[(dataset_2021['Deceduto'] == -1) | (dataset_2021['Deceduto'] == '-1')],
...
dataset_columns = dataset_columns.loc[dataset_columns['NaturaIncidente'].isin(['Veicolo in marcia che urta buche nella carreggiata', 'Veicolo in marcia contro ostacolo fisso', 'Veicolo in marcia contro ostacolo accidentale', 'Veicolo in marcia contro treno', 'Veicolo in marcia contro animale'])]
...
accidents_data_frame.to_csv('dataset/processed/scatterPlot/Data/scatterPlotNatureC5-2021.csv', header=True)